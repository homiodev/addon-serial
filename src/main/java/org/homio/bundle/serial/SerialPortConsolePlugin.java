...
public class SerialPortConsolePlugin implements ConsolePluginCommunicator {
  ...
  private void reopen(Boolean open) {
    if (this.rawSerialPortCommunicator != null) {
      ...
      context.ui().toastr().info("serial.port_closed", Map.of("PORT", portName));
    ...
    if (open) {
      ...
      if (commPort == null) {
        ...
        throw new ServerException("serial.no_port", Map.of("PORT",
      ...
      try {
        ...
        context.setting().setValue(ConsoleHeaderSerialOpenPortSetting.class, false);
        throw new ServerException("serial.unable_open", Map.of("PORT", commPort.getSystemPortName()));
      }
    }
  }
  ...
  private void openPort(SerialPort commPort) {
    ...
    if (!opened) {
      ...
      context.ui().toastr().success("serial.open_success", Map.of("PORT", commPort.getSystemPortName()));
    }
  }
